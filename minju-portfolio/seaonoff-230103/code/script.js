(function(){
    ...
    function resetFn(){
        ...
        if(innerWidht < 1250) innerWidht = 1200;
        document.querySelectorAll('.img').forEach((elem, index)=>{
            elem.style.transform = `rotateY(calc(var(--i)*21.17deg)) translateZ(${innerWidht/2.8}px)`;
        });
    }
    ...
    function ringAnifn(){
        ...
        ring.style.transform = ` rotateY(${ringRotate-=0.02}deg)`;
    ...
    document.querySelector('body').addEventListener('mousemove',(e)=>{
       ...
       lastMove = e.clientX;
       ringRotate =Number((ring.style.transform).split('Y(')[1].split('deg')[0]);
       let rotatemove = (startx + e.clientX )/(8000); 
       if( (lastMove-firstMove) < 0){
          ring.style.transform = ` rotateY(${ringRotate-(rotatemove*1.5)}deg)`;
        }else ring.style.transform = ` rotateY(${ringRotate+(rotatemove*1.8)}deg)`;
